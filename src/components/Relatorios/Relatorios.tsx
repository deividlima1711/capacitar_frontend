...
const Relatorios: React.FC = () => {
  ...
  const tarefasConcluidasArr = dadosFiltrados.tarefas.filter(t => t.status === 'concluido' && t.tempoGasto > 0);
  const tempoMedioExecucao = tarefasConcluidasArr.length > 0 ?
    (tarefasConcluidasArr.reduce((acc, t) => acc + t.tempoGasto, 0) / tarefasConcluidasArr.length).toFixed(1) : '0';
